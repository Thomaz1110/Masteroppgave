...
class ESKFMultiRobot:
    ...
    def __init__(self, dt, sigma_acc, sigma_bias, sigma_vel, sigma_range, num_robots):
        ...
        for robot in range(num_robots):
            initiator_offset = robot * 6
            noise_offset = robot * 4
            
            # Continuous-time system matrix
            self.Ac[initiator_offset + 0, initiator_offset + 2] = 1.0
            self.Ac[initiator_offset + 1, initiator_offset + 3] = 1.0
            self.Ac[initiator_offset + 2, initiator_offset + 4] = -1.0
            self.Ac[initiator_offset + 3, initiator_offset + 5] = -1.0
            self.Ac[initiator_offset + 4, initiator_offset + 4] = decay
            self.Ac[initiator_offset + 5, initiator_offset + 5] = decay

            # Continuous-time process noise matrix
            self.Ec[initiator_offset + 2, noise_offset + 0] = -1.0
            self.Ec[initiator_offset + 3, noise_offset + 1] = -1.0
            self.Ec[initiator_offset + 4, noise_offset + 2] = 1.0
            self.Ec[initiator_offset + 5, noise_offset + 3] = 1.0
        ...
        Q_block = np.diag([q_acc, q_acc, q_b, q_b])
        self.Qd = np.kron(np.eye(num_robots), Q_block)
    ...
    def update(self, robot_index, update_type, y_meas, y_ins_hat, initiator_nom=None, reflector_nom=None, reflector_index=None):
        ...
        if update_type == "velocity_x":
            ...
            C[0, initiator_offset + 0] = diff[0] / dist
            C[0, initiator_offset + 1] = diff[1] / dist
