...
def sim_accelerometer(acc_true, dt, T_acc=300.0, sigma_bias=9.6e-4, sigma_acc=9.6e-3, seed=None, initial_bias_seed=None):
  rng = np.random.default_rng(seed)
  initial_bias_rng = np.random.default_rng(initial_bias_seed)
  N = len(acc_true) 
  bias = np.zeros((N, 2)) 
  bias0 = initial_bias_rng.uniform(-0.1, 0.1, size=2)
  bias[0] = bias0
  ...
  for k in range(1, N):
    bias[k] = bias0 + phi * (bias[k - 1] - bias0) + np.sqrt(Qb) * rng.normal(size=2)
